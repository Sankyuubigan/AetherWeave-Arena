...
func _init():
    ...
    # ИСПРАВЛЕНИЕ: Генерируем круглую текстуру (мягкую кисть)
    ...
    circle_tex.fill_from = Vector2(0.5, 0.5) # Из центра
    circle_tex.fill_to = Vector2(1.0, 0.5)   # К краю
    circle_tex.width = 64
    ...
    draw_mat.albedo_texture = circle_tex # Применяем нашу круглую текстуру
